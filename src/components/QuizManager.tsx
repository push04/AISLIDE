...
function GlassRibbonButton({
  ...
  return (
    <button
      {...props}
      className={[
        'inline-flex items-center justify-center gap-2 px-4 py-2 rounded-2xl text-sm font-semibold text-slate-800',
        'bg-white/80 hover:bg-white shadow-sm border border-slate-200',
        'active:scale-[.985] transition-all focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400',
...
function ProgressSlimLight({ value, label }: { value: number; label: string }) {
  return (
    <div className="min-w-[320px]">
      <div className="text-xs text-slate-700 mb-1 text-center">{label}</div>
...
export const QuizManager: React.FC<QuizManagerProps> = ({ uploads, apiKey }) => {
  ...
  const generateQuiz = useCallback(async () => {
    ...
    try {
      const api = new OpenRouterAPI(apiKey);
      const raw = await api.generateQuiz(upload.fullText, questionCount);
    ...
    return (
      <div className="space-y-8">
        <div className="text-center">
          <h1 className="text-3xl font-extrabold text-slate-900">Quiz Summary</h1>
          <p className="text-slate-600">Great job! Review your results and jump back in.</p>
        </div>

        {/* Light ribbon (summary) */}
        <div className="sticky top-[64px] z-30 border-b border-slate-200 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80">
          <div className="app-container py-3 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2 text-sm">
              <div className="px-3 py-2 rounded-xl border border-slate-200 bg-white/80 text-slate-700 flex items-center gap-2">
        ...
        <div className="flex flex-wrap gap-3">
          <button
            ...
            disabled={!wrongCount}
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-indigo-600 text-white font-semibold shadow-sm hover:bg-indigo-700 disabled:bg-slate-400 transition-all"
          >
          ...
          <button
            ...
            className="inline-flex items-center gap-2 px-4 py-2 rounded-xl bg-amber-500 text-white font-semibold shadow-sm hover:bg-amber-600 disabled:bg-slate-400 transition-all"
          >
        ...
        <div className="bg-white border rounded-xl p-6 shadow-sm">
          <h2 className="text-xl font-bold text-slate-900 mb-4">Review Answers</h2>
              ...
              return (
                <div key={i} className="rounded-lg border p-4">
                  <div className="flex items-center justify-between mb-3">
                    ...
                    <span className={`inline-flex items-center gap-1 text-sm rounded-full px-2 py-0.5 ${isRight ? 'bg-emerald-100 text-emerald-700' : 'bg-rose-100 text-rose-700'}`}>
        ...
        <div className="bg-white border rounded-xl p-6 shadow-sm">
          ...
          {!reportLoading && !reportError && (
            <div className="rounded-lg border bg-slate-50 p-4 text-slate-800 whitespace-pre-wrap leading-relaxed">
              {reportText || 'Report will appear here.'}
            </div>
          )}
        </div>
      </div>
      ...
      <div className="space-y-8">
        ...
        <div className="sticky top-[64px] z-30 border-b border-slate-200 bg-white/95 backdrop-blur supports-[backdrop-filter]:bg-white/80">
          <div className="app-container py-3 flex items-center justify-between gap-3">
            ...
            <div className="flex items-center gap-2">
              ...
              <GlassRibbonButton
                onClick={toggleFlag}
                className={session.flagged[session.currentIndex] ? 'ring-2 ring-amber-300 bg-amber-50' : ''}
        ...
        <div className="app-container">
          <div className="flex flex-wrap items-center gap-3 mb-4">
            <div className="text-sm font-semibold text-slate-700">Question Palette</div>
            ...
            <div className="ml-auto flex items-center gap-2 text-xs text-slate-500">
              <span className="inline-flex items-center gap-1">
                <span className="w-3 h-3 rounded bg-white border border-slate-300 inline-block" /> Unanswered
              </span>
              <span className="inline-flex items-center gap-1">
                <span className="w-3 h-3 rounded bg-emerald-50 border border-emerald-300 inline-block" /> Answered
              </span>
              <span className="inline-flex items-center gap-1">
                <span className="w-3 h-3 rounded bg-amber-200 inline-block" /> Flagged
              </span>
            </div>
          </div>
        </div>
        ...
        <div className="bg-white border rounded-xl p-6 shadow-sm">
          ...
          <div className="space-y-3">
            {current.options.map((opt, i) => {
              ...
              const base =
                'group w-full text-left px-4 py-4 rounded-xl border transition-all duration-200 focus:outline-none focus-visible:ring-2 focus-visible:ring-indigo-400';
              const cls = showRight
                ? 'border-emerald-300 bg-emerald-50'
                : showWrong
                ? 'border-rose-300 bg-rose-50'
          ...
          {reveal && current.explanation && (
            <div className="mt-5 rounded-xl border bg-slate-50 p-4 text-slate-700">
  ...
  return (
    <div className="space-y-8">
      <div className="text-center">
        <h1 className="text-3xl font-extrabold text-slate-900">Generate & Take Quizzes</h1>
        <p className="text-slate-600">Pick a processed document, generate a quiz, and start practicing in seconds.</p>
      </div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4 mb-6">
          <div>
            ...
            <select
              ...
              className="st-select w-full px-3 py-2 border rounded-lg focus:ring-2 focus:ring-indigo-500"
            >
          ...
          <div className="flex items-end">
            <button
              ...
              className="w-full md:w-auto inline-flex items-center justify-center gap-2 px-6 py-3 rounded-xl bg-indigo-600 text-white font-semibold shadow-sm hover:bg-indigo-700 active:scale-[.99] disabled:bg-slate-400 transition-all"
            >
              {generating ? (
                <>
