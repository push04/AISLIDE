  ...
  return (
    <div className="space-y-8">
      ...
      <div className="text-center space-y-4">
        ...
        <h1 className="text-4xl font-bold text-slate-800">Welcome to SlideTutor AI</h1>
          ...
          {quickActions.map((action, index) => {
            ...
            return (
              <button
                ...
                className="p-6 rounded-lg border border-slate-200 hover:border-indigo-300 hover:shadow-md transition-all duration-200 text-left group"
              >
      ...
      {/* Recent Uploads */}
      <div className="bg-white rounded-xl p-8 shadow-sm border border-slate-200">
        <h2 className="text-2xl font-bold text-slate-800 mb-6">Recent Uploads</h2>
        {uploads.length === 0 ? (
          <div className="text-center py-12">
            <Upload className="w-12 h-12 text-slate-400 mx-auto mb-4" />
            <p className="text-slate-600 mb-4">No uploads yet. Get started by uploading your first document!</p>
            <button
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {uploads.slice(-6).reverse().map((upload) => (
              <div key={upload.id} className="border border-slate-200 rounded-lg p-4 hover:shadow-md transition-shadow">
                <div className="flex items-start gap-3">
                  <div className="w-10 h-10 bg-slate-100 rounded-lg flex items-center justify-center">
                    <FileText className="w-5 h-5 text-slate-600" />
                  </div>
                  <div className="flex-1 min-w-0">
                    <h3 className="font-semibold text-slate-800 truncate">{upload.filename}</h3>
