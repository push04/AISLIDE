...
const UploadDropzone = memo(({ onFilesAdded }: { onFilesAdded: (files: File[]) => void }) => {
    ...
    return (
        ...
        >
            ...
            <h3 className="text-xl font-semibold text-slate-800">Drop files or click to upload</h3>
            <p className="text-sm text-slate-600">PDF and PPTX supported</p>
...
const FileListItem = memo(({ entry, onCancel, onRemove, onRetry }: { entry: UploadEntry; onCancel: () => void; onRemove: () => void; onRetry: () => void; }) => {
    ...
    const getStatusInfo = () => {
        switch (status) {
            case "pending": return { icon: <Clock className="w-5 h-5 text-slate-500" />, text: "Pending...", color: "bg-slate-50 text-slate-700" };
            case "uploading": return { icon: <Loader2 className="w-5 h-5 animate-spin text-indigo-600" />, text: "Uploading...", color: "bg-indigo-50 text-indigo-700" };
            case "processing": return { icon: <Loader2 className="w-5 h-5 animate-spin text-sky-600" />, text: "Processing...", color: "bg-sky-50 text-sky-700" };
            case "success": return { icon: <CheckCircle className="w-5 h-5 text-emerald-600" />, text: "Success", color: "bg-emerald-50 text-emerald-700" };
            case "error": return { icon: <AlertCircle className="w-5 h-5 text-rose-600" />, text: "Error", color: "bg-rose-50 text-rose-700" };
            case "cancelled": return { icon: <XCircle className="w-5 h-5 text-slate-400" />, text: "Cancelled", color: "bg-slate-50 text-slate-500" };
        }
    };
            ...
            <div className="flex items-center gap-4 min-w-0">
                ...
                <div className="flex-1 min-w-0">
                    ...
                    <p className="text-xs text-slate-500">{formatBytes(file.size)}</p>
                     ...
                     {status === "error" && <p className="text-xs text-rose-600 mt-1 truncate" title={errorMessage}>{errorMessage}</p>}
                </div>
            </div>
            <div className="flex items-center gap-2 shrink-0">
                <span className={`hidden sm:inline-flex items-center gap-2 px-2 py-1 text-xs rounded-full ${color}`}>{icon}{text}</span>
                 {status === "uploading" && <button onClick={onCancel} title="Cancel" className="p-1.5 text-slate-500 hover:bg-slate-100 rounded-md"><X className="w-4 h-4" /></button>}
                 {(status === "error" || status === "cancelled") && <button onClick={onRetry} title="Retry" className="p-1.5 text-slate-500 hover:bg-slate-100 rounded-md"><RotateCcw className="w-4 h-4" /></button>}
                 {(status === "error" || status === "cancelled" || status === "success") && <button onClick={onRemove} title="Remove" className="p-1.5 text-rose-500 hover:bg-rose-50 rounded-md"><Trash2 className="w-4 h-4" /></button>}
            </div>
        </div>
    );
});
...
const UploadedFileItem = memo(({ upload, onDelete }: { upload: Upload; onDelete: () => void; }) => (
    <div className="p-4 flex items-center justify-between gap-4 bg-slate-50/50">
        <div className="flex items-center gap-4 min-w-0">
            <div className="w-12 h-12 bg-white border rounded-lg flex items-center justify-center shrink-0">
                <FileText className="w-6 h-6 text-slate-600" />
            </div>
            <div className="min-w-0">
                <p className="font-medium text-slate-800 truncate">{upload.filename}</p>
                <p className="text-xs text-slate-500">{formatBytes(upload.size ?? 0)} • Uploaded on {new Date(upload.uploadedAt).toLocaleDateString()}</p>
            </div>
        </div>
        <div className="flex items-center gap-2 shrink-0">
  ...
  return (
    <div className="space-y-6 max-w-4xl mx-auto">
      <div className="text-center p-4">
        ...
        <p className="text-slate-600 mt-1">Uploads will start automatically. Concurrency: {concurrency}.</p>
      </div>

      <div className="bg-white rounded-xl p-6 shadow-sm border border-slate-200">
        <UploadDropzone onFilesAdded={addFilesToQueue} />
        {globalError && <div className="mt-3 text-sm text-center text-rose-600 p-2 bg-rose-50 rounded-md">{globalError}</div>}
      </div>

      {/* Upload Queue Section */}
      <div className="bg-white rounded-xl shadow-sm border border-slate-200">
        <div className="p-4 border-b border-slate-200">
            <h2 className="text-lg font-semibold text-slate-800">Upload Queue</h2>
        </div>
